...
const fn generate_shortest_keyboard_costs<const DEPTH: usize>(
    ...
    loop {
        ...
        loop {
            answer[start][end] = cacher.get_shortest_path_between_numerics::<DEPTH>(start, end);
...
impl ArrowCacher {
    const fn get_shortest_path_between_numerics<const DEPTH: usize>(
        &mut self,
        start: usize,
        end: usize,
    ) -> u64 {
        ...
        loop {
            ...
            let mine = self.get_numeric_path_min_cost::<DEPTH>(shortest[shortest_index]);
            if mine < best {
                best = mine;
            }
    ...
    const fn get_numeric_path_min_cost<const DEPTH: usize>(&mut self, numeric_path: Path) -> u64 {
        return self.get_arrow_path_min_cost(DEPTH, numeric_path);
    }

    const fn get_arrow_path_min_cost(&mut self, depth: usize, path: Path) -> u64 {
        ...
        loop {
            ...
            loop {
                let path_cost = self.get_arrow_path_min_cost(depth - 1, paths[path_index]);
                path_index += 1;
